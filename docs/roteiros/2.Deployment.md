...
??? **DICAS**
    ...
    No arquivo de views.py de cada Django modifique-os
    ...
    def index(request):
        ...
        message = 'Server2 (ou Server3) is live, current time is: '
        return Response(data=message + date, status=status.HTTP_200_OK)

    No arquivo de urls.py adicionar o path no urlpatterns
...
!!! exercise 
    ...
    1. Altere o conteúdo da mensagem contida na função `index` do arquivo `tasks/views.py` de cada server para distinguir ambos os servers. Faça um `GET request` para cada um dos servidores e tire um print da resposta de cada request, provando que voce está conectado ao server 4, que é o Proxy Reverso.  
