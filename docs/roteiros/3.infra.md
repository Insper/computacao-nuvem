...
Esse roteiro está atualizado da documentação oficial, porém, em caso de proeblemas, consulte a documentação oficial antes mesmo de pedir ajuda, pois lá irá conter a solução para a maioria dos problemas.
...
The ceph-osd application is deployed to four nodes with the [ceph-osd](https://charmhub.io/ceph-osd){target=_blank} charm.
...
``` {.yaml .copyable .numberLines startFrom="1"}
ceph-osd:
