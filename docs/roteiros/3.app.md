...
O Django deve ser implantado em duas máquinas virtuais (VMs), executando a mesma aplicação em paralelo, garantindo alta disponibilidade.

O acesso do usuário deve ser feito por meio de um balanceador de carga com proxy reverso (NGINX), que distribuirá as requisições entre as duas instâncias da aplicação Django.
