...
Abaixo está o fluxograma do andamento da disciplina:
...
flowchart LR
 ...
 subgraph subGraph3["Módulo 4"]
    ...
    TE1 -- 70% --> n1
    T1 -- 50% --> n1
    n1 --> EX1[["Exam 1 - Arguição 1"]]
    EX1 --> n2["🔒"]
    R2 -- 100% --> n2
    TE2 -- 70% --> n2
    T2 -- 50% --> n2
    n2 --> EX2[["Exam 2 - Arguição 2"]]
    EX2 --> n3["🔒"]
    R3 -- 100%--> n3
    TE3 --70%--> n3
    T3 --50%--> n3
    n3 --> EX3[["Exam 3 - Arguição 3"]]
    EX3 --> n4["🔒"]
    R4 --100%--> n4
    TE4 -- 70%--> n4
    T4 -- 50%--> n4
    n4 --> EX4[["Exam 4 - Arguição 4"]]
...
O progresso do curso só ocorre quando o aluno completa as três trilhas e passa por uma arguição (Exam) na plataforma PrairieLearn correspondente ao módulo. Essas arguições funcionam como barreiras de progressão, seguindo o modelo Assessment Locked, onde a aprovação é condição necessária para liberar o acesso à próxima arguição, porém o aluno pode percorrer as atividades  Tutoriais e Teoria como achar mais proveitoso em seus estudos de modo a facilitar como cada individuo trata a disciplina.
...
Essa abordagem permite ao aluno vivenciar uma experiência educacional que integra teoria, prática e aplicação de conceitos de infraestrutura e desenvolvimento em nuvem de forma progressiva, com forte acompanhamento e checkpoints claros de aprendizado.
...
Esta nota será calculada com base na média das quatro arguições (Exams) realizadas ao longo do semestre.
Importante: a participação na Arguição 4 é **obrigatória**. O aluno que não fizer a última arguição estará automaticamente reprovado, independentemente do desempenho nas anteriores.

Nota em Grupo (50% da nota final):
Corresponde ao desempenho no Projeto Final, que será desenvolvido em equipe.
Importante: a entrega do Projeto Final é obrigatória. A não entrega do projeto resultará em reprovação na disciplina, independentemente das demais notas.
...
|---|---|---|
| **Média das Arguições Individuais** | **50%** | Média das 4 arguições. A **Arguição 4 é obrigatória**: ausência resulta em reprovação independente das demais notas. |
| **Projeto Final (Trabalho em Grupo)** | **50%** | Entrega obrigatória. **A não entrega reprova automaticamente o aluno**, independentemente da nota nas arguições. |
| **Bônus 1 – Exercícios Teóricos (100%)** | **+1 ponto na média das arguições** | Aluno que atingir **100% de acerto em todos os exercícios teóricos dos 4 módulos** recebe 1 ponto extra na média das arguições. |
| **Bônus 2 – Conclusão de Todos os Tutoriais** | **+1 ponto na nota do projeto** | Aluno que concluir **todos os 8 tutoriais práticos dos 4 módulos** recebe **1 ponto extra na sua nota individual no Projeto Final**. |

## Observações Gerais

- A disciplina adota o modelo de progressão por domínio (Mastery Learning).
- O Projeto Final e a Arguição 4 são componentes eliminatórios.
