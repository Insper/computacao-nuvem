...
Agora realize as seguinte modificações (se preciso, leia os manuais):
  ...
  * Modifique o DHCP server para utilizar a subrede 192.168.0.0/20.
  ...
  1. Quantos IPs utilizáveis estão disponíveis na subrede 192.168.0.0/20? Todos os IP são utilizáveis? \vspace{4cm}
  1. Qual a diferença entre um IP público e um IP privado? \vspace{6cm}
  1. Qual a classe utilizada na rede interna do Insper? E na sua rede? Quantas classes existem? \vspace{7cm}


# Criando a Infraestrutura (Nuvem Bare-metal)

* Consulte o diagrama no arquivo "H1 - Diagrama"
...
**Atenção**: verificar se as interfaces de rede se chamam eth0 (placa rede padrão com PXE) e eth1 (NIC usb). Verificar o Global Kernel Parameters, apagar o nó e reiniciar o processo caso algo esteja errado.
  ...
  1. Descreva o processo PXE Boot? Qual a sua grande vantagem em um datacenter real? \vspace{6cm}
  1. Analisando em um aspecto mais amplo, quais outras funcionalidades do MaaS podem ser úteis no gerenciamento de *bare metal*? \vspace{6cm}
...
## Finalizando
  ...
  1. O que é e como funciona o NAT? \vspace{8cm}
  1. O que é e como funciona a VPN? \vspace{8cm}
  1. O que deveria ser feito para você conseguir acessar o Maas da sua casa sem VPN? \vspace{4cm}

## Questões Complementares

  1. O que significa LTS? Por que isso importa para uma empresa? \vspace{5.5cm}
  1. O que é IPv6? Qual a importância da migração? \vspace{5.5cm}
  1. A literatura preconiza que o Modelo de Rede Internet possui 5 camadas, quais são elas e quais camadas foram envolvidas nesse capítulo? \vspace{6.5cm}
  1. A literatura mais antiga discorre sobre o Modelo de Rede OSI de 7 camadas. Explique a diferença entre os dois modelos. \vspace{10cm}

## Concluindo

  1. O que é e para que serve um gerenciador de Bare Metal? \vspace{8cm}
  1. O que é um MAC address? \vspace{8cm}
  1. O que é um IP address? Como ele difere do MAC address? \vspace{8cm}
  1. O que é CIDR? Qual o papel da subrede? \vspace{8cm}
  1. O que são DHCP, DNS e gateway? \vspace{8cm}
